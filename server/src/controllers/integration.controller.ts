...
export class IntegrationController {
   ...
   */
  static async getDevConfig(req: Request, res: Response): Promise<void> {
    try {
      ...
      // Catalog counts
      const [
        realCategories, sandboxCategories,
        realItems, sandboxItems,
        realVariants, sandboxVariants,
        realAddons, sandboxAddons,
        realInventory, sandboxInventory
      ] = await Promise.all([
      ...
      const mappingsQuery: any = { tenantId, isDeleted: false };
      const parsedOutletId = req.query.outletId as string;
      ...
      const integrationProviders = (OrderGatewayService as any).getRegisteredProviders
        ? (OrderGatewayService as any).getRegisteredProviders()
        : ["MOCK_SWIGGY", "MOCK_ZOMATO", "QR", "WEBSITE"];

      // Stats
      const [
        totalOrders, sandboxOrders,
        totalExternalOrders, sandboxExternalOrders,
        eventQueueCount, sandboxEventQueueCount,
        syncJobsCount, sandboxSyncJobsCount
      ] = await Promise.all([
   ...
   */
  static async loadDemoCatalog(req: Request, res: Response): Promise<void> {
    try {
      const tenantId = req.user?.tenantId;
      if (!tenantId) {
        ApiResponseHandler.unauthorized(res, "User not authenticated or tenantId not found");
      ...
      const categoryIdMap: Record<string, Types.ObjectId> = {};
      ...
      for (const cat of categoriesSeed) {
        ...
        categoryIdMap[cat.name] = categoryDoc._id as Types.ObjectId;
      }
   ...
   */
  static async generateMappings(req: Request, res: Response): Promise<void> {
    try {
      const tenantId = req.user?.tenantId;
      if (!tenantId) {
        ApiResponseHandler.unauthorized(res, "User not authenticated or tenantId not found");
      ...
      const providers = (OrderGatewayService as any).getRegisteredProviders
        ? (OrderGatewayService as any).getRegisteredProviders()
        : ["MOCK_SWIGGY", "MOCK_ZOMATO", "QR", "WEBSITE"];
      const extOutletId = "6a3c17666bb70afe757e4a91";
      ...
      for (const provider of providers) {
        await ChannelOutletMapping.findOneAndUpdate(
          { tenantId: tenantObjectId, provider, externalOutletId: extOutletId },
          { outletId: outletObjectId, isActive: true, isSandbox: true, sandboxVersion: "v1", isDeleted: false },
      ...
      for (let idx = 0; idx < items.length; idx++) {
        ...
        for (const provider of providers) {
          ...
          if (provider === "MOCK_SWIGGY") providerPrefix = "SWG";
          else if (provider === "MOCK_ZOMATO") providerPrefix = "ZMT";
          else if (provider === "WEBSITE") providerPrefix = "WEB";
          
          const extItemId = `${providerPrefix}_ITEM_${numStr}`;

          // MenuItem Mapping
          await ChannelMenuItemMapping.findOneAndUpdate(
            { tenantId: tenantObjectId, outletId: outletObjectId, provider, externalItemId: extItemId },
            { menuItemId: item._id, isActive: true, isSandbox: true, sandboxVersion: "v1", isDeleted: false },
            { upsert: true, new: true }
          );
          mappedItems++;
        }
        ...
        for (let vIdx = 0; vIdx < variants.length; vIdx++) {
          ...
          for (const provider of providers) {
            ...
            await ChannelVariantMapping.findOneAndUpdate(
              { tenantId: tenantObjectId, outletId: outletObjectId, provider, externalVariantId: extVarId },
              { menuItemId: item._id, variantId: v._id, isActive: true, isSandbox: true, sandboxVersion: "v1", isDeleted: false },
              { upsert: true, new: true }
            );
            mappedVariants++;
          }
        ...
        for (let aIdx = 0; aIdx < addons.length; aIdx++) {
          ...
          for (const provider of providers) {
            ...
            await ChannelAddonMapping.findOneAndUpdate(
              { tenantId: tenantObjectId, outletId: outletObjectId, provider, externalAddonId: extAddonId },
              { menuItemId: item._id, addonId: a._id, isActive: true, isSandbox: true, sandboxVersion: "v1", isDeleted: false },
              { upsert: true, new: true }
            );
            mappedAddons++;
          }
        ...
        if (item.sku === "SANDBOX-ITEM-001") {
          ...
          for (const provider of ["MOCK_SWIGGY", "MOCK_ZOMATO"]) {
            await ChannelMenuItemMapping.findOneAndUpdate(
              { tenantId: tenantObjectId, outletId: outletObjectId, provider, externalItemId: "1001" },
              { menuItemId: item._id, isActive: true, isSandbox: true, sandboxVersion: "v1", isDeleted: false },
              { upsert: true, new: true }
            );
            mappedItems++;
          ...
          for (const v of variants) {
            if (v.name === "Regular") {
              for (const provider of ["MOCK_SWIGGY", "MOCK_ZOMATO"]) {
                await ChannelVariantMapping.findOneAndUpdate(
                  { tenantId: tenantObjectId, outletId: outletObjectId, provider, externalVariantId: "V201" },
                  { menuItemId: item._id, variantId: v._id, isActive: true, isSandbox: true, sandboxVersion: "v1", isDeleted: false },
                  { upsert: true, new: true }
                );
                mappedVariants++;
          ...
          for (const provider of ["MOCK_SWIGGY", "MOCK_ZOMATO"]) {
            await ChannelMenuItemMapping.findOneAndUpdate(
              { tenantId: tenantObjectId, outletId: outletObjectId, provider, externalItemId: "2002" },
              { menuItemId: item._id, isActive: true, isSandbox: true, sandboxVersion: "v1", isDeleted: false },
              { upsert: true, new: true }
            );
            mappedItems++;
          ...
          for (const a of addonsList) {
            if (a.name === "Jalapenos") {
              for (const provider of ["MOCK_SWIGGY", "MOCK_ZOMATO"]) {
                await ChannelAddonMapping.findOneAndUpdate(
                  { tenantId: tenantObjectId, outletId: outletObjectId, provider, externalAddonId: "A402" },
                  { menuItemId: item._id, addonId: a._id, isActive: true, isSandbox: true, sandboxVersion: "v1", isDeleted: false },
                  { upsert: true, new: true }
                );
                mappedAddons++;
   ...
   */
  static async validateMappings(req: Request, res: Response): Promise<void> {
    try {
      const tenantId = req.user?.tenantId;
      if (!tenantId) {
        ApiResponseHandler.unauthorized(res, "User not authenticated or tenantId not found");
      ...
      const brokenVariants: any[] = [];
      const brokenAddons: any[] = [];

      const mappedItemIds = new Set<string>();
      const mappedVariantIds = new Set<string>();
      const mappedAddonIds = new Set<string>();
   ...
   */
  static async resetDevSandbox(req: Request, res: Response): Promise<void> {
    try {
      const tenantId = req.user?.tenantId;
      if (!tenantId) {
        ApiResponseHandler.unauthorized(res, "User not authenticated or tenantId not found");
   ...
   */
  static async simulateOrder(req: Request, res: Response): Promise<void> {
    try {
      const tenantId = req.user?.tenantId;
      if (!tenantId) {
        ApiResponseHandler.unauthorized(res, "User not authenticated or tenantId not found");
   ...
   */
  static async stopSimulatorSession(req: Request, res: Response): Promise<void> {
    try {
   ...
   */
  static async getSimulatorMetrics(req: Request, res: Response): Promise<void> {
    try {
      const rawSessionId = req.params.sessionId;
      const sessionId = Array.isArray(rawSessionId) ? String(rawSessionId[0]) : String(rawSessionId || "");
      if (!sessionId || !Types.ObjectId.isValid(sessionId)) {
   ...
   */
  static async getSimulatorEvents(req: Request, res: Response): Promise<void> {
    try {
      const rawSessionId = req.params.sessionId;
      const sessionId = Array.isArray(rawSessionId) ? String(rawSessionId[0]) : String(rawSessionId || "");
      if (!sessionId || !Types.ObjectId.isValid(sessionId)) {
   ...
   */
  static async getSimulatorSessions(req: Request, res: Response): Promise<void> {
    try {
      const tenantId = req.user?.tenantId;
      if (!tenantId) {
        ApiResponseHandler.unauthorized(res, "User not authenticated or tenantId not found");
