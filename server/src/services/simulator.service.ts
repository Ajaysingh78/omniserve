...
import SimulationSession, { ISimulationSession, ISimulationJob } from "../models/simulationsession.model.js";
...
interface StartSimulationInput {
  ...
  totalOrders?: number | undefined; // For BURST
  durationMinutes?: number | undefined; // For CONTINUOUS (max 30)
  speed: "REALTIME" | "FAST" | "STRESS_TEST";
  chaosMode: boolean;
...
export class SimulatorService {
  ...
  private static activeSessions = new Map<string, {
    ...
    trackerIntervalId?: NodeJS.Timeout;
   ...
   */
  static async startSimulation(input: StartSimulationInput): Promise<ISimulationSession> {
    ...
    this.activeSessions.set((session._id as Types.ObjectId).toString(), sessionState);
    ...
    this.startBackgroundTracker((session._id as Types.ObjectId).toString(), input.tenantId);
    ...
    if (input.mode === "CONTINUOUS") {
      ...
      const stopTimeout = setTimeout(async () => {
        await this.stopSimulation((session._id as Types.ObjectId).toString());
      }, durationMs);
   ...
   */
  private static async runJobAsynchronously(
    ...
    if (!sessionState || sessionState.stopRequested) return;

    // Update job status to RUNNING
    await SimulationSession.updateOne(
      { _id: session._id, "jobs._id": job._id },
      {
        $set: {
          "jobs.$.status": "RUNNING",
    ...
    const runIterationStep = async (iteration: number) => {
      ...
      if (iteration >= job.totalIterations) {
        ...
        if (freshSession && freshSession.jobs.every(j => j.status === "COMPLETED" || j.status === "CANCELLED" || j.status === "FAILED")) {
          ...
          await this.logEvent({
            ...
            sessionId: freshSession._id as Types.ObjectId,
            eventType: "SESSION_COMPLETED",
            details: { reason: "All iterations completed" }
          });

          await SimulationMetricsService.persistSessionMetrics(freshSession._id as Types.ObjectId);
      ...
      await SimulationSession.updateOne(
        { _id: session._id, "jobs._id": job._id },
        { $set: { "jobs.$.currentIteration": iteration + 1 } }
      );
   ...
   */
  private static async simulateOrderStep(args: {
    tenantId: string;
    outletId: string;
    provider: string;
    ...
    let chaos: string | undefined = undefined;
    if (args.chaosMode) {
    ...
    const processedOrder = await OrderGatewayService.processExternalOrder({
      externalOrderId: (externalOrder._id as Types.ObjectId).toString(),
      tenantId: args.tenantId
    });

    // Link all internal orders, events, sync jobs, and timeline records
    await this.linkSessionData(
      args.sessionId,
      args.tenantId,
      args.outletId,
      orderId,
      processedOrder.internalOrderId?.toString()
    );
   ...
   */
  private static startBackgroundTracker(sessionId: string, tenantId: string) {
    const sessionState = this.activeSessions.get(sessionId);
   ...
   */
  private static cleanupSession(sessionId: string) {
    const sessionState = this.activeSessions.get(sessionId);
    if (!sessionState) return;
   ...
   */
  private static async linkSessionData(
    sessionId: Types.ObjectId,
    tenantId: string,
    outletId: string,
    ...
    await ExternalOrder.updateMany(
      { tenantId: new Types.ObjectId(tenantId), externalOrderId, sessionId: null },
      { $set: { sessionId } }
    );
   ...
   */
  private static async logEvent(args: {
   ...
   */
  private static getIntervalMs(speed: string): number {
    switch (speed) {
      case "FAST":
        return 300; // 300ms
      case "STRESS_TEST":
