...
import SimulationSession, { ISimulationSession } from "../models/simulationsession.model.js";
...
export interface IAggregatedMetrics {
  ...
  throughput: number; // orders/sec
  syncThroughput: number; // syncs/sec
  queueDepth: number; // pending events + sync jobs
  retries: number;
...
export class SimulationMetricsService {
   ...
   */
  static async getMetrics(sessionId: string | Types.ObjectId): Promise<IAggregatedMetrics | null> {
   ...
   */
  static async persistSessionMetrics(sessionId: string | Types.ObjectId): Promise<ISimulationSession | null> {
    const sessionObjectId = typeof sessionId === "string" ? new Types.ObjectId(sessionId) : sessionId;
    const aggregated = await this.getMetrics(sessionObjectId);
    if (!aggregated) return null;

    return await SimulationSession.findByIdAndUpdate(
      sessionObjectId,
      {
